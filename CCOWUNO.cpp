...
CCOWUNO::CCOWUNO(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6, uint8_t ID7) : OneWireItem(ID1, ID2, ID3, ID4, ID5, ID6, ID7)
{
...
void CCOWUNO::duty(OneWireHub * const hub)
{
...
void CCOWUNO::clearMemory(void)
{
    memset(memory, static_cast<uint8_t>(0x00), sizeof(memory));
}

void CCOWUNO::clearScratchpad(void)
{
    memset(scratchpad, static_cast<uint8_t>(0x00), SCRATCHPAD_SIZE);
}

bool CCOWUNO::writeMemory(const uint8_t* const source, const uint8_t length, const uint8_t position)
{
...
bool CCOWUNO::readMemory(uint8_t* const destination, const uint16_t length, const uint16_t position) const
{
...
void CCOWUNO::setPageProtection(const uint8_t position)
{
...
bool CCOWUNO::getPageProtection(const uint8_t position) const
{
...
void CCOWUNO::setPageEpromMode(const uint8_t position)
{
...
bool CCOWUNO::getPageEpromMode(const uint8_t position) const
{
    if      (position < 1*PAGE_SIZE)
    {
...
bool CCOWUNO::updatePageStatus(void)
{
    page_eprom_mode = 0;
    page_protection = 0;
